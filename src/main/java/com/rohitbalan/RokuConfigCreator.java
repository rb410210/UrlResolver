...
public class RokuConfigCreator {
	...
	private void browseCategory(Category category) throws Exception {
		...
		// System.out.println(category.getName() + " (" + channels.size() + "):
		// " + channels);
	...
	private void splitXMLs(List<Channel> routedChannels, Category category, String routedOrDirect) throws Exception {
		if (routedChannels.size() > 0) {
			...
			for (int channelIndex = 0; channelIndex < routedChannels.size(); channelIndex = channelIndex + 8) {
				List<Channel> sublist = routedChannels.subList(channelIndex,
						(channelIndex + 8) > routedChannels.size() ? routedChannels.size() : (channelIndex + 8));
				new XMLWriter().createXML(
						category.getName() + " " + routedOrDirect + (channelIndex == 0 ? "" : channelIndex / 8),
						sublist);
			}
		}
	...
	private List<Channel> traverseChannels(Document html) throws IOException {
		...
		for (Element anchorTag : anchorTags) {
			if ("clip-link".equals(anchorTag.attr("class")) && anchorTag.getElementsByTag("span").size() > 0
				...
				for (Element iframe : iframes) {
					...
					if (iframe.hasAttr("src")
							&& originalVideoUrl.startsWith("http://oklivetv.com/xplay/xplay.php?idds=")) {
						String parsedVideoUrl = new OktvService()
								.getHtmlContentFromUrl("http://192.168.0.102:9888/urlresolver/?id="
										+ originalVideoUrl.replace("http://oklivetv.com/xplay/xplay.php?idds=", ""));
	...
	private List<Category> traverseCategories(List<Element> elements) {
		...
		for (Element listItem : elements) {
			...
			if (listItem.hasAttr("class") && listItem.attr("class").contains("cat-item")) {
				// System.out.println(listItem.getElementsByTag("ul").size() +
				// listItem.html());
				if (listItem.getElementsByTag("ul").size() > 0) {
					categories
							.addAll(traverseCategories(listItem.getElementsByTag("ul").first().getElementsByTag("li")));
				} else {
